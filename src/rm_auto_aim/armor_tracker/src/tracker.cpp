...
void Tracker::updateStableVYaw()
{
  // 当跟踪稳定且状态为TRACKING时，更新稳定角速度值
  if (tracker_state == TRACKING && lost_count_ == 0) {
    // 当前角速度与之前估计的角速度差异小于阈值时认为是稳定的
    double current_v_yaw = target_state(7);
    if (!has_stable_v_yaw_) {
      stable_v_yaw_ = current_v_yaw;
      has_stable_v_yaw_ = true;
    } else if (std::abs(current_v_yaw - stable_v_yaw_) < v_yaw_stability_threshold_) {
      // 使用指数移动平均来平滑更新稳定值
      stable_v_yaw_ = 0.9 * stable_v_yaw_ + 0.1 * current_v_yaw;
    } else if (std::abs(current_v_yaw - stable_v_yaw_) > v_yaw_stability_threshold_) {
      // 使用指数移动平均来平滑更新稳定值
      stable_v_yaw_ = 0.9 * stable_v_yaw_ + 0.1 * current_v_yaw;
    }
...
void Tracker::initEKF(const Armor & a)
{
  ...
  // //如果有稳定的角速度值，则使用它
  // double initial_v_yaw = has_stable_v_yaw_ ? stable_v_yaw_ : 0.0;
  // //状态向量填充: x, vx, y, vy, z, vz, yaw, v_yaw, r
  // target_state << xc, 0, yc, 0, za, 0, yaw, initial_v_yaw, r;
