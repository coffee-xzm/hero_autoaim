...
public:
  ...
  //绘图相关辅助
  const std::vector<cv::Point3f> & getSmallArmorPoints() const { return small_armor_points_; }
  const std::vector<cv::Point3f> & getLargeArmorPoints() const { return large_armor_points_; }
