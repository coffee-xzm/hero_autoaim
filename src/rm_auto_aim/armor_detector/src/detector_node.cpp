  ...
  if (pnp_solver_ != nullptr) {
    ...
    for (const auto & armor : armors) {
      ...
      if (success) {
        ...
        //RCLCPP_INFO(get_logger(), "tilt = %f",armor.left_light.tilt_angle+armor.right_light.tilt_angle)/2);
        ...
        //add
        armor_msg.angle=(float)(armor.left_light.tilt_angle+armor.right_light.tilt_angle)/2;
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  if (debug_) 
  {
    ...
    //otherdraws
    cv::Point2f vertices_light1[4],vertices_light2[4];
    for (const auto & armor : armors){
      ...
      if (pnp_solver_->solvePnP(armor, rvec, tvec)) {
        ...
        auto object_points = armor.type == ArmorType::SMALL ? 
                             pnp_solver_->getSmallArmorPoints() : 
                             pnp_solver_->getLargeArmorPoints();
        ...
        cv::projectPoints(
          object_points, 
          rvec, tvec, 
          pnp_solver_->getCameraMatrix(), 
          pnp_solver_->getDistCoeffs(), 
          projected_points
        );
