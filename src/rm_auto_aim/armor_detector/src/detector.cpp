...
std::vector<Light> Detector::findLights(const cv::Mat & rbg_img, const cv::Mat & binary_img)
{
    ...
    for (const auto & contour : contours)
    {
        ...
        // ensure the b_rect in the image
        if (b_rect.x >= 0 && b_rect.y >= 0 && b_rect.x + b_rect.width <= rbg_img.cols - 1 &&
            b_rect.y + b_rect.height <= rbg_img.rows - 1)
        {
            // To avoid the case that a round is detected as a light(3.14*r^2 / 4 * r^2 = 0.785)
...
bool Detector::isLight(const Light & light)
{
    ...
    // try not to get the light that will result in the roi of number to be expanded
    bool size_ok = light.length >= 13 && light.width >= 4;
...
ArmorType Detector::isArmor(const Light & light_1, const Light & light_2)
{
    ...
    // To ensure the roi is not from expand
    float point_distance_top = cv::norm(light_1.top - light_2.top);
    float point_distance_bottom = cv::norm(light_1.bottom - light_2.bottom);
    bool armor_roi_ok = point_distance_top > 32 && point_distance_bottom > 32;
    ...
    bool is_armor = light_ratio_ok && center_distance_ok && angle_ok && armor_roi_ok;
...
void Detector::reextractArmors(std::vector<Armor> & armors)
{
    for(auto & armor : armors)
    {
        ...
        // 根据原本的图像提取出左右灯条的roi
        cv::Mat left_roi = gray_img(cv::Rect(left_point_lu, left_point_rb));
        cv::Mat right_roi = gray_img(cv::Rect(right_point_lu, right_point_rb));
        // 二值化
