/**
 * @file    light_corner_corrector.cpp
 * @brief   交点重新提取节点
 *
 * @author  ZhangHaopeng
 * @date    2024/6/20
...
void LightCornerCorrector::correctCorners(Armor &armor, const cv::Mat &gray_img) {
  ...
  if (armor.left_light.width > PASS_OPTIMIZE_WIDTH) {
    ...
    if(left_axis.mean_val == 0) return;

    Refind_Axis_Direction(left_axis);
  ...
  if (armor.right_light.width > PASS_OPTIMIZE_WIDTH) {
    ...
    if(right_axis.mean_val == 0) return;

    Refind_Axis_Direction(right_axis);
    // Find the corner of the light
...
void LightCornerCorrector::Refind_Axis_Direction(SymmetryAxis &axis)
{
  if(axis.direction.y > 0)
  {
...
SymmetryAxis LightCornerCorrector::findSymmetryAxis(const cv::Mat &gray_img, const Light &light) {
  ...
  // using gamma from the light to enhance the image
  // 先除以255，再进行gamma增强，再乘以255
  roi.convertTo(roi, CV_32F, 1.0 / 255);
  cv::pow(roi, light.enhancing_gamma, roi);
  roi *= 255;
  ...
  for (int i = 0; i < roi.rows; i++) {
      for (int j = 0; j < roi.cols; j++) {
          auto val = std::round(roi.at<float>(i, j));
  ...
  if(points.size() == 0)
  {
    return SymmetryAxis{.centroid = centroid, .direction = cv::Point2f(0, 0), .mean_val = 0};
  }
...
cv::Point2f LightCornerCorrector::findCorner(const cv::Mat &gray_img,
  ...
  for (int i = -half_n; i <= half_n; i++) {
    ...
    for (float x = x0 + dx, y = y0 + dy; distance(x, y, x0, y0) < L * (END - START);
      ...
      // using gamma enhancement for each points
      double brightness_diff = pow(gray_img.at<uchar>(cv::Point(prev))* 1.0f / 255 , light.enhancing_gamma) * 255 -
                               pow(gray_img.at<uchar>(cv::Point(cur))* 1.0f / 255 , light.enhancing_gamma) * 255 ;
      if (brightness_diff > max_brightness_diff && pow(gray_img.at<uchar>(prev)* 1.0f / 255 , light.enhancing_gamma) * 255 > axis.mean_val) {
